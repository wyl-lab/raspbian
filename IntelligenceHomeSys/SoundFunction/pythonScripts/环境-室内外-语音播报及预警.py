...
def get_weather(url,data):
        ...
        outdoorStatus = "\n---------[室外环境监测情况]------------"
        result = date +"好，管家为您播报。" +outdoorStatus+ "\n蓬莱天气，"+weather_status+"，\n白天到夜间，"+ \
...
#-----------------------------------------------------------
        homeStatus = "\n\n---------[室内环境监测情况]------------"
        '''
        homeTemp = getTemp_home()
        homeWater = getHumidness_home()
        homeGas = getGasConcentration_home()
        homeSecurity = getSafeStatus_home()
        '''
        homeTemp = "26.6"
        homeWater = "41.7"
        homeGas = "[警报]可燃气/烟雾浓度超出安全范围！！"
        homeSecurity = "[警报]检测到未知人员！！" #暂未检测到有人员活动
        homeStatus += "\n温度：" + homeTemp + "摄氏度，" + "\n湿度：" + homeWater + "%，" + \
                      "\n煤气浓度：" + homeGas + "，\n安防系统：" + homeSecurity + "\n"

        return result+homeStatus
